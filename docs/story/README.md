...
- **Implemented:** the piece reactions in [04](04-animation-reactions.md), in 2D: mood gestures, Hanzō's smoke vanish, the ninja's mounted leap, the promotion flip, death by armor, facing the attacker, trembling ashigaru, hanging-piece pulse, ink pin lines, the promotion glint, and the kneeling lord with dusk at checkmate (`components/Board.tsx`, `story/reactions.ts`).
- **Story page** (`#/story`, `components/Story.tsx` + `story/cast.ts`): the cast codex, built from the game's own sprites and lines, with the intro, sound cues and the history note. Two keynote slides ("The night of Iga", "How the pieces talk") link to it.
- **Not yet implemented** (polish): the row of fallen pieces beside the board, the scene ambience by game state (04 §7), and the board tremor on heavy landings.
