...
| Path | What | Owner |
|---|---|---|
| `backend/engine/` | Rust UCI engine `chezz` (shakmaty movegen; search + eval are ours) | backend |
| `backend/runner.py` | Plays chezz vs Stockfish (`UCI_LimitStrength` + `UCI_Elo`, 5 s/move each), saves games | backend |
...
```bash
cd backend/engine && cargo build --release          # engine → target/release/chezz.exe
echo bench | backend/engine/target/release/chezz.exe  # speed/depth sanity check
